"""Rumi quadruped hardware interface — position control mode.
...
Motor ID → joint name mapping (from README):
    1  FL_hip_joint      2  FL_thigh_joint    3  FL_calf_joint
    4  BL_hip_joint      5  BL_thigh_joint    6  BL_calf_joint
    7  BR_hip_joint      8  BR_thigh_joint    9  BR_calf_joint
    10 FR_hip_joint      11 FR_thigh_joint    12 FR_calf_joint

Typical usage:
    found = scan_motors()
    print(found)            # e.g. [1, 2, 3, ...]

    hw = MX64Hardware(motor_id=1)
    hw.connect()
    hw.enable()
...
JOINT_TO_MOTOR_ID: dict[str, int] = {v: k for k, v in MOTOR_ID_TO_JOINT.items()}
...
DEFAULT_KP = 6.0    # Nm/rad  (written on every enable())
...
def scan_motors(
    ...
    Prints each found ID and its joint name (if in MOTOR_ID_TO_JOINT).
    """
    ...
    for mid in id_range:
        ...
        if result == COMM_SUCCESS:
            joint = MOTOR_ID_TO_JOINT.get(mid, "unknown")
            print(f"[scan] Found motor ID {mid:2d}  ({joint})")
            found.append(mid)
...
class MX64Hardware:
    ...
    def connect(self) -> None:
        ...
        print(f"[MX64] Connected on {self._device} @ {self._baudrate} baud.")
