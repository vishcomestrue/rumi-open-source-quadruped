"""Motor assembly sim2real — single joint pendulum load.

Spawns the motor_assembly.xml in sim and a ghost updated from real MX-64 hardware.
User commands a target angle via GUI slider; same command goes to both sim and real.
...
───────────────────
  ...
  Main process:
      ...
      Viz  thread  (CONTROL_HZ): viser update + live plots
      Rec  thread  (CONTROL_HZ): data recording (starts on Connect, saves on stop)
      Main thread  (60 Hz)     : GUI poll

Run
───
  python load_sim2real.py
  python load_sim2real.py --control-hz 100 --device /dev/ttyUSB0
"""
...
DEFAULT_TARGET       = 0.0   # rad

# ── Shared memory ───────────────────────────────────────────────────────────────
# shm_q_real   : 1 double — ghost joint angle (offset-corrected)
# shm_dq_real  : 1 double — real joint velocity (rad/s)
# shm_tau_real : 1 double — real joint torque (Nm)
# shm_q_sim    : 1 double — sim joint angle (rad)
# shm_dq_sim   : 1 double — sim joint velocity (rad/s)
# shm_tau_sim  : 1 double — sim actuator force (Nm)
# shm_ctrl     : 1 double — target angle commanded from GUI
# shm_real_hz  : 1 double — real loop rate
# shm_sim_hz   : 1 double — sim loop rate
# shm_viz_hz   : 1 double — viz loop rate
...
def real_process_fn(
    ...
    try:
        hw.connect()
        hw.enable(kp=DEFAULT_KP, kd=DEFAULT_KD)
        ...
        while not stop_event.is_set():
            ...
            # Command is offset from initial position (absolute ticks to motor)
...
def sim_thread_fn(
    ...
    dt          = 1.0 / CONTROL_HZ
    n_substeps  = max(1, round(dt / model.opt.timestep))  # steps per control tick
    iters  = 0
    t_rate = time.time()
    ...
    while not stop_event.is_set():
        ...
        # Publish sim state for recorder + viz
...
def recorder_thread_fn(
    ...
    out_path = _DATA / f"{ts}_load_sysid.npz"
...
def viz_thread_fn(
    ...
    while not stop_event.is_set():
        ...
        # Update 3D views from shared sim state
...
def main():
    parser = argparse.ArgumentParser(description="Motor assembly sim2real")
    parser.add_argument("--control-hz", type=int, default=CONTROL_HZ)
    parser.add_argument("--device", type=str, default="/dev/ttyUSB0",
                        help="Serial port for MX-64 (e.g. /dev/ttyUSB0)")
    parser.add_argument("--record", action="store_true",
                        help="Record data to NPZ on each connect")
    ...
    real_proc = mp.Process(
        ...
        args=(args.control_hz, args.device,
              ...
              shm_ctrl, shm_real_hz, connect_mp, stop_mp),
        daemon=True,
    )
    real_proc.start()
    ...
    server    = viser.ViserServer(label="Motor Sim2Real")
    ...
    with server.gui.add_folder("Control"):
        sl_target = server.gui.add_slider(
            "Target angle (rad)",
            min=-2 * np.pi, max=2 * np.pi, step=0.01,
            initial_value=DEFAULT_TARGET,
        )

    with server.gui.add_folder("Physics params"):
        sl_kp           = server.gui.add_slider("kp",            min=0.0, max=120.0, step=0.1,   initial_value=DEFAULT_KP)
    ...
    rec_connect  = threading.Event()  # mirrors connect_mp for threads
    ...
    if args.record:
        ...
        rec_thread.start()
        print("Recording enabled (--record): data will be saved on connect.")
    else:
        rec_thread = None
        print("Recording disabled. Use --record to enable.")

    # ── Main loop (60 Hz) ─────────────────────────────────────────────────────
    print("Running. Ctrl+C to stop.")
    try:
        while True:
            _np(shm_ctrl)[0] = sl_target.value

            with _phys_lock:
                _phys_params["kp"]           = sl_kp.value
                _phys_params["kd"]           = sl_kd.value
            ...
            # Mirror connect event to recorder thread
            if connect_mp.is_set() and not rec_connect.is_set():
                rec_connect.set()
